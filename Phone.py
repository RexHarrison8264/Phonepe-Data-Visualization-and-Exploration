...
# cloning the Github Pulse repository
os.system("git clone https://github.com/PhonePe/pulse.git")

# Creating connection with sqlite

connection = sqlite3.connect('Phonepay.db')
cursor = connection.cursor()


#Setting-up page configuration

icon = Image.open ('C:/Users/User/Desktop/Phonepay.jpg')
st.set_page_config (page_title = "PhonePe Data Virtualization Dashboard",
                    page_icon= icon,
                    layout = "wide",
...
st.sidebar.header(":rainbow: :violet[**Hello! Welcome to the dashboard**]")
...
with st.sidebar:
    selected= option_menu("Menu",["HOME","Top Charts","Explore Data","About"],
              icons=["house-check","graph-up-arrow","database-check","file-person"],
...
if selected == 'HOME':
    st.header(':violet[Data Visualization and Exploration]', divider='rainbow')
    st.subheader(":violet[A User-Friendly Tool Using Streamlit and Plotly] :sunglasses:")
    st.write(" ")
    st.write(" ")
    st.subheader(":violet[Domain :] Fintech")
    st.subheader(":violet[Technologies used :] Github Cloning, Python, Pandas, MySQL, mysql-connector-python, Streamlit, and Plotly.")
    st.subheader(":violet[Overview :] In this streamlit web app you can visualize the phonepe pulse data and gain lot of insights on transactions, number of users, top 10 state, district, pincode and which brand has most number of users and so on. Bar charts, Pie charts and Geo map visualization are used to get some insights.")
...
# Overall State Data - TRANSACTIONS AMOUNT - INDIA MAP
        with col1:
            ...
            file_path= 'C:/Users/User/Desktop/Statenames.csv '
            df1 = pd.DataFrame(cursor.fetchall(),columns= ['State', 'Total_Transactions', 'Total_amount'])
            df2 = pd.read_csv(f'C:/Users/User/Desktop/Statenames.csv')
            df1.State = df2
